...
// boxfill.webp — THE PATTERN WITH NO LOGIN BOX IN IT, cut to the box's own
// neighbourhood. The box the source draws is a 215px SQUARE, and the login row
// is a 430 x 87 rectangle now (see lib/loginBox.ts), so the square has to go:
// its white paper and its red outline both stand outside the new rectangle,
// top and bottom, and would read as a notch cut out of the cloud field.
//
// It is the same mirror fill edge.webp already uses — the strip left of the box
// reflected into its left half, the strip right of it into its right — so the
// join is seamless on all four sides and the only hard seam is the vertical one
// down the middle, which the new rectangle covers. Cut at the frames' own
// WIDTH and sharpen, so laying it over a frame is the same pixels at the same
// weight and the patch cannot show as a rectangle of its own.
//
// edge.webp is the same picture and could not be used for this: it is written
// at 900 wide with sharpen 0.4, and scaled back to 640 its lines carry a
// visibly different weight (mean |diff| 32 of 255 against f100 outside the box
// at all, measured) — a patch has to match what it is patching.
const FILL = { ...BOX_R };
const fillBuf = Buffer.from(acc);
process(fillBuf, LAST, 0); // the final state, black already at 0
cloudFill(fillBuf, Buffer.from(fillBuf), FILL, 1);
const fpng = new PNG({ width: W, height: H });
fillBuf.copy(fpng.data);
const FH = Math.round((H * WIDTH) / W);
const fx0 = Math.floor(FILL.x0 * WIDTH), fy0 = Math.floor(FILL.y0 * FH);
const fx1 = Math.ceil(FILL.x1 * WIDTH), fy1 = Math.ceil(FILL.y1 * FH);
await sharp(PNG.sync.write(fpng))
  .resize({ width: WIDTH })
  .sharpen({ sigma: 0.5 })
  .extract({ left: fx0, top: fy0, width: fx1 - fx0, height: fy1 - fy0 })
  .webp({ quality: 95 })
  .toFile('public/splash/boxfill.webp');
/*
 * Where it goes back down. The crop rounds outward to whole pixels of the
 * frames' own WIDTH, so the fractions the page draws it at are those pixels
 * over the frame, not FILL's own — half a pixel of slip here is a seam right
 * across the middle of the design.
 */
const BOXFILL_AT = {
  x0: fx0 / WIDTH, x1: (fx1) / WIDTH, y0: fy0 / FH, y1: (fy1) / FH,
};
...
for (const f of ['edge', 'settle', 'boxfill', 'blackbox', 'phone-label']) stamp.update(readFileSync(`public/splash/${f}.webp`));
...
writeFileSync(
  ...
  `// GENERATED by scripts/build-splash-frames.mjs — do not edit.\n` +
    ...
    '/** Where boxfill.webp is laid back down, as fractions of the frame. */\n' +
    `export const SPLASH_BOXFILL = ${JSON.stringify(BOXFILL_AT)};\n` +
...
console.log(`wrote ${written} frames + edge/settle/boxfill/blackbox/phone-label.webp + lib/splashEdgeProfile.ts (${edgeBuf.length}B)`);
