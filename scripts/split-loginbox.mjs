 ...
 * dashed lines) into one SVG with a <g> per part:
 *   #label-email #cloud-email #line-email …  (all black — NO red border)
 *   → public/splash/loginbox-parts.svg
 *
 *   npm run build:splash    (runs this too)
 *
 * This is the login form overlay only. The red outline box comes from the baked
 * animation frame, not from here. The output viewBox is the content bounding box
 * + a symmetric pad. It also prints the row geometry to paste into
 * SPLASH_GEOM.rows in lib/splashFrames.ts.
 */
...
// No #border — the red outline is the baked animation box; this file is the
// black form only.
const out =
